...
### Multi-Region Deployment with External IAM Management

For multi-region deployments, it's recommended to manage IAM roles externally in your parent module rather than having each ECS cluster create its own IAM resources. This approach provides centralized IAM management and avoids duplicate resources across regions.

```hcl
# Parent module managing multiple regions
module "label" {
  source = "cloudposse/label/null"
...
# Create IAM role once for all regions
resource "aws_iam_role" "ecs_role" {
  name = "${module.label.id}-ecs-role"
...
module "ecs_cluster_primary" {
  ...
  capacity_providers_ec2 = {
    default = {
      instance_type      = "t3.medium"
      security_group_ids = [module.vpc_primary.vpc_default_security_group_id]
      subnet_ids         = module.subnets_primary.private_subnet_ids
      min_size           = 0
      max_size           = 2
    }
  }
...
module "ecs_cluster_secondary" {
  ...
  # Use the same externally managed IAM role
  create_iam_role        = false
  existing_iam_role_name = aws_iam_role.ecs_role.name
  ...
  capacity_providers_ec2 = {
    default = {
      instance_type      = "t3.medium"
      security_group_ids = [module.vpc_secondary.vpc_default_security_group_id]
      subnet_ids         = module.subnets_secondary.private_subnet_ids
      min_size           = 0
      max_size           = 2
    }
  }
...
For complete multi-region examples, see:
- [examples/ec2-multi-region-global](examples/ec2-multi-region-global) - EC2 capacity provider with external IAM
- [examples/ec2-multi-region-secondary](examples/ec2-multi-region-secondary) - EC2 capacity provider using existing IAM
- [examples/fargate-multi-region-global](examples/fargate-multi-region-global) - Fargate capacity provider with external IAM  
- [examples/fargate-multi-region-secondary](examples/fargate-multi-region-secondary) - Fargate capacity provider using existing IAM
...
| <a name="input_create_iam_role"></a> [create\_iam\_role](#input\_create\_iam\_role) | Whether to create IAM role and instance profile for ECS instances | `bool` | `true` | no |
...
| <a name="input_existing_iam_role_name"></a> [existing\_iam\_role\_name](#input\_existing\_iam\_role\_name) | Name of existing IAM role to use when create\_iam\_role is false | `string` | `null` | no |
...
| <a name="output_role_instance_profile"></a> [role\_instance\_profile](#output\_role\_instance\_profile) | IAM instance profile name |
<!-- markdownlint-restore -->

## Testing

This module includes comprehensive test coverage for all functionality.

### Prerequisites

Before running tests, you need to install the required dependencies:

#### Install bats (for bash-based tests)
...
#### Install Go (for integration tests)

**Requirement:** Go 1.19 or later
...
bats --version  # Should show bats version
```

### Running All Tests

```bash
# Run all tests (module validation + examples)
make all

# Run module validation tests only
make module
...
cd test/src && go test -v

# Run specific test suites
cd test/src && go test -v -run TestExamplesEC2MultiRegion
cd test/src && go test -v -run TestExamplesFargateMultiRegion
cd test/src && go test -v -run TestExamplesIAMValidation
```

### Test Coverage

- **Module Validation**: Basic Terraform validation, linting, and formatting
- **Example Testing**: Validates all example configurations
- **Multi-Region Testing**: Tests external IAM management patterns
- **IAM Validation Testing**: Tests input validation and error scenarios
